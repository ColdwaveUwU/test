...
class TesterImp {
    ...
    async checkModalWindow() {
        await this.frame.waitForFunction(() => {
            const modalWindow = document.querySelector(".modals-mask");
            const isModalWindowOpen = modalWindow ? modalWindow.checkVisibility() : false;
            return !isModalWindowOpen;
        }, {});
    }

    /**
     * Retrieves the current number of open modal windows from the DOM.
     * @returns {Promise<number>} The current modal window count. Returns 0 if the counter is not available or invalid.
     */
    async getModalCounter() {
        try {
            return await this.frame.evaluate(() => {
                try {
                    const el = document.querySelector(".modals-mask");
                    if (!el) {
                        return 0;
                    }

                    const attr = el.getAttribute("counter");
                    const counter = Number(attr);
            ...
            throw new Error(`getModalCounter: failed to evaluate in frame. ${error.message}`, { cause: error });
        }
    }
     ...
     */
    waitModalWindowClosed(modalCounter = 1) {
        if (modalCounter <= 0) {
            throw new Error(`Invalid modal counter: ${modalCounter}. Cannot wait for counter below 0.`);
        }

        const targetCounter = String(modalCounter - 1);
        return this.frame.waitForFunction(
            (expectedCounter) => {
                ...
                const counter = mask.getAttribute("counter");
