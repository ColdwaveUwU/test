...
class OcisFileMenu extends FileMenu {
    constructor(tester) {
        super(tester);
    }

    static OCIS_FILEMENU_SELECTORS = { ...selectors, ...FileMenu.FILEMENU_SELECTORS };
    ...
    async #renameBeforeSaving(newFileName) {
        const renameSelectors = OcisFileMenu.OCIS_FILEMENU_SELECTORS.NEW_NAME;
        if (newFileName) {
    ...
    async saveCopyAs(format, newFileName = "", encode = "Unicode (UTF-8)") {
        try {
            const saveCopyAsSelectos = OcisFileMenu.OCIS_FILEMENU_SELECTORS.PANEL_MENU.SAVE_COPY_AS;
